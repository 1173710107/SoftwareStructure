...
public class TrackGame extends ConcreteCircularOrbit<Athlete, Athlete> {
  ...
  /**
   * 增加一个新的轨道 AF:新建一个轨道，对应于一个跑道 RI：true immutable.
   */
  ...
  /**
   * 删除掉最外层的轨道 AF:删除最外层的一个轨道，对应于体育场上的跑道 RI：true immutable.
   */
   ...
   * 通过读取的文本信息构建track
   * 
   * AF:将文件内容string进行分析转化成的是具体各个数据类型的数据 RI：true immutable.
   */
  public void Analysis() {
    ...
    try {
      for (String info : infoall) {
        ...
        Pattern p3 =
            Pattern.compile("Athlete ::= <([a-zA-z]+),(\\d+),([A-Z]{3}),(\\d+),(\\d{1,2}.\\d{2})>");
        ...
        if (m3.find()) {
          ...
          athletes.add(new Athlete(m3.group(1), Integer.valueOf(m3.group(2)), m3.group(3),
              Integer.valueOf(m3.group(4)), Double.valueOf(m3.group(5))));
          visited = true;
        }
        if (!visited) {
          ...
          if (line.length == 5) { // 运动员
              ...
              if (line1[0].equals("Athlete ::= ")) {
                ...
                if (sp.length == 1) {
                  ...
                  if (sp1.length == 1) { // 不够两位自动补0
                    ...
                    String newinfo1 = line[0] + "," + line[1] + "," + line[2] + "," + line[3] + ","
                        + sp1[0] + "." + sp1[1] + "0>";
                    Pattern p7 = Pattern.compile(
                        "Athlete ::= <([a-zA-z]+),(\\d+),([A-Z]{3}),(\\d+),(\\d{1,2}.\\d{2})>");
                    ...
                    if (m7.find()) {
                      ...
                      athletes.add(new Athlete(m7.group(1), Integer.valueOf(m7.group(2)),
                          m7.group(3), Integer.valueOf(m7.group(4)), Double.valueOf(m7.group(5))));
                      visited = true;
            ...
            } else if (line1[0].equals("NumOfTracks ")) {
  ...
  /**
   * 进行随机的排序 AF：对应于对轨道上的运动员进行分配跑道 RI：true immutable.
   */
  ...
  /**
   * 按照成绩进行排序 AF:对应于对跑道上的运动员进行分配跑道 RI：true immutable.
   */
