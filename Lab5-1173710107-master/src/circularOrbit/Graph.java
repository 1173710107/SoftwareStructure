...
/**
 * A mutable weighted directed graph with labeled vertices. Vertices have distinct labels of an
 * immutable type {@code L} when compared using the {@link Object#equals(Object) equals} method.
 * Edges are directed and have a positive weight of type {@code int}.
 * 
 * <p>
 * PS2 instructions: this is a required ADT interface. You MUST NOT change the specifications or add
 * additional methods.
 * 
...
public interface Graph<L> {
   ...
   * @return true if this graph did not already include a vertex with the given label; otherwise
   *         false (and this graph is not modified)
   */
  public boolean add(L vertex);

  /**
   * Add, change, or remove a weighted directed edge in this graph. If weight is nonzero, add an
   * edge or update the weight of that edge; vertices with the given labels are added to the graph
   * if they do not already exist. If weight is zero, remove the edge if it exists (the graph is not
   * otherwise modified).
   * 
   ...
   * @return true if this graph included a vertex with the given label; otherwise false (and this
   ...
   *         an edge from that vertex to target, and the value for each key is the (nonzero) weight
   *         of the edge from the key to target
   */
  public Map<L, Integer> sources(L target);

  /**
   * Get the target vertices with directed edges from a source vertex and the weights of those
   * edges.
   * 
