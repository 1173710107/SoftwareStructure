...
public class CircularOrbitAPIs<L, E> {
   ...
   * @param e1 开始物体e1
   * @param e2 终结点物体e2
   * @return 返回合法的距离，如果未获取到距离，返回-1 AF:逻辑距离在系统中是相隔几个轨道 RI：true mutable
   */
   ...
   * @param e1 开始点的物体e1
   * @param e2 终结点的物体e2
   * @return 返回距离，如果不符合要求，返回-1 AF:利用两点距离公式计算对应的物理距离 RI：true mutable
   */
   ...
   * @param c1 第一个系统c1 
   * @param c2 第二个系统c2
   * @return 包括每个轨道数目不同的一个list AF:两个参数对于各个数据的不同 RI：true mutable
   */
   ...
   * 获取整个系统的熵值.
   * 
   * @param nums 每个轨道上分布物体的数量
   * @return 整个的熵 AF:计算熵值 RI：true mutable
   */
   ...
   * 
   * @param a 一个系统a
   * @return 该系统的熵 AF:计算整个系统熵值 RI：true mutable
   */
